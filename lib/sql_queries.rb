...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) WHERE (SUM(pladges.amount) > projects.funding_goal) FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title"
end
