...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE (SUM(pladges.amount) > projects.funding_goal) GROUP BY projects.title"
end
